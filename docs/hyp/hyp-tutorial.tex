...
\newcommand{\hypcompose}{\texttt{hyp Compose}\xspace}
...
$ cat sent.hyp
\input{fig/sent1.hyp}
$ \textbf{hyp Compose} cfg.hyp sent.hyp
...
The \code{hyp Union} command takes the union of two or more
...
\code{hyp Concat} concatenates two or more hypergraphs. If you
use \code{Concat} instead of \code{Union} in the example above,
you obtain the output shown in \tabref{concat_result}.
\examplecols{concat_result}{Concat result.}{0.4}{1.7cm}

\subsection{Other \hyp commands}

The \hyp toolkit also provides the following commands, in addition
to the ones discussed above:

\begin{itemize}
\item \code{Complement} creates the complement of an unweighted
  finite-state hypergraph
\item \code{ConvertStrings} converts string input into string
  hypergraphs (i.e., hypergraphs like the one shown in
  \tabref{sent1})
\item \code{Determinize} determinizes a finite-state hypergraph
  (currently only unweighted acceptors allowed)
\item \code{Draw} draws a hypergraph in \code{dot} format (see the
  figures in this tutorial)
\item \code{Empty} decides if a hypergraph is empty, i.e., has no
  valid paths
\item \code{GetString} prints the string contained in a one-path
  hypergraph
\item \code{Prune} prunes away all unreachable arcs of a hypergraph
\item \code{Reverse} reverses a hypergraph
\item \code{Reweight} modifies arc weights of a hypergraph
\item \code{SamplePath} generates sample paths from a hypergraph
\end{itemize}
