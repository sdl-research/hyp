...
/**
   ...
   it appears that a block has a pointer and size_t of overhead (so two pointers
   worth, or 16 bytes, on 64-bit).
*/
...
#ifndef POOL__POOL_HPP
#define POOL__POOL_HPP
#pragma once

#ifndef POOL_MALLOC_BLOCK_TARGET
#define POOL_MALLOC_BLOCK_TARGET 8128
/**

   set by looking at TBB malloc source:

   fittingAlignment = cache line size (for avoiding false sharing) is 64 bytes for
   at least the past few years of intel (since pentium D netburst, which had 128)

   see post at the end of http://software.intel.com/en-us/forums/topic/292721

   slabSize=16kb slabs (also aligned to 16kb):


   #define SET_FITTING_SIZE(N) ( (slabSize-sizeof(Block))/N ) & ~(fittingAlignment-1)
   // For blockSize=16*1024, sizeof(Block)=2*estimatedCacheLineSize and
   fittingAlignment=estimatedCacheLineSize,
...
#include <Pool/simple_segregated_storage.hpp>
// boost::alignment_of
...
/*!
  \file
  \brief Provides class \ref pool: a fast memory allocator that guarantees proper alignment of all allocated
  chunks,
...
struct default_user_allocator_new_delete {
  ...
  static char* malloc BOOST_PREVENT_MACRO_SUBSTITUTION(
      const size_type bytes) {  //! Attempts to allocate n bytes from the system. Returns 0 if out-of-memory
    return new (std::nothrow) char[bytes];
  }
  static void free BOOST_PREVENT_MACRO_SUBSTITUTION(char* const block) {  //! Attempts to de-allocate block.
...
struct default_user_allocator_malloc_free {
  ...
  static void free BOOST_PREVENT_MACRO_SUBSTITUTION(char* const block) { (std::free)(block); }
};

namespace details {  //! Implemention only.

template <typename SizeType>
class PODptr {  //! PODptr is a class that pretends to be a "pointer" to different class types
...
*/
 ...
 private:
  ...
  char* ptr_next_ptr() const {
    return (ptr_next_size() - ::boost::math::static_lcm<sizeof(size_type), sizeof(void*)>::value);
  }
 ...
 public:
  ...
  PODptr() : ptr(0), sz(0) {  //! default constructor for PODptr will result in an invalid object.
  }

  bool valid() const {  //! A PODptr object is either valid or invalid.
    //! An invalid PODptr is analogous to a null pointer.
    //! \returns true if PODptr is valid, false if invalid.
    return (begin() != 0);
  }
  void invalidate() {  //! Make object invalid.
    begin() = 0;
  }
  char*& begin() {  //! Each PODptr keeps the address and size of its memory block.
    //! \returns The address of its memory block.
    return ptr;
  }
  char* begin() const {  //! Each PODptr keeps the address and size of its memory block.
    //! \return The address of its memory block.
    return ptr;
  }
  char* end() const {  //! \returns begin() plus element_size (a 'past the end' value).
    return ptr_next_ptr();
  }
  size_type total_size() const {  //! Each PODptr keeps the address and size of its memory block.
  ...
  char*& next_ptr() const {  //! \returns pointer to next pointer area.
  ...
  static PODptr sort(PODptr list) {
    if (!list.valid() || !list.next().valid()) return list;
  ...
  static PODptr merge(PODptr listA, PODptr listB) {
    ...
    if (std::less<void*>()(listB.ptr, listA.ptr)) std::swap(listA, listB);
...
/*!
  ...
  This parameter may be 0 (the default), in which case we try to allocate
  as close as possible to POOL_MALLOC_BLOCK_TARGET
  ...
  There are essentially two ways to use class pool: the client can call \ref malloc() and \ref free() to
  allocate
  and free single chunks of memory, this is the most efficient way to use a pool, but does not allow for
  the efficient allocation of arrays of chunks.  Alternatively, the client may call \ref ordered_malloc() and
  \ref
  ordered_free(), in which case the free list is maintained in an ordered state, and efficient allocation of
  arrays
...
class pool : protected simple_segregated_storage<typename UserAllocator::size_type> {
 ...
 protected:
  ...
  static bool is_from(void* const chunk, char* const i,
                      const size_type sizeof_i) {  //! \param chunk chunk to check if is from this pool.
  ...
  size_type alloc_size()
    ...
    // sizeof(size_type)),
    // but is now more parsimonious: just rounding up to the minimum required alignment of our housekeeping
    // data
  ...
  static void*& nextof(void* const ptr) {  //! \returns Pointer dereferenced.
  ...
  */
  void target_alloc_size(size_type bytes_from_allocator) {
    ...
    if (alloc < block_max)
      ...
      start_size = next_size = 1;
  }

  // pre: npartition_size != 0 && nnext_size != 0
  explicit pool(const size_type nrequested_size, const size_type nnext_size = 0, const size_type nmax_size = 0)
      : list(0, 0)
    ...
    if (nnext_size)
      ...
      target_alloc_size(POOL_MALLOC_BLOCK_TARGET);

    set_max_size(nmax_size);
  }

  ~pool() {  //!   Destructs the Pool, freeing its list of memory blocks.
    purge_memory();
  }
  ...
  size_type get_max_size() const {  //! \returns max_size.
    return max_size;
  }
  void set_max_size(const size_type nmax_size) {  //! Set max_size.
    BOOST_USING_STD_MIN();
    max_size = nmax_size ? min BOOST_PREVENT_MACRO_SUBSTITUTION(nmax_size, max_chunks()) : max_chunks();

    set_next_size(next_size);
  }
  size_type get_requested_size() const {  //!   \returns the requested size passed into the constructor.
  ...
  // Returns 0 if out-of-memory
  void* malloc
  BOOST_PREVENT_MACRO_SUBSTITUTION() {  //! Allocates a chunk of memory. Searches in the list of memory blocks
  ...
  void* ordered_malloc() {  //! Same as malloc, only merges the free lists, to preserve order. Amortized O(1).
    //! \returns a free chunk from that block.
    //! If a new memory block cannot be allocated, returns 0. Amortized O(1).
    // Look for a non-empty storage
    if (!store().empty()) return (store().malloc)();
  ...
  void free BOOST_PREVENT_MACRO_SUBSTITUTION(void* const chunk) {  //!   Deallocates a chunk of memory. O(1).
    //!
  ...
  // pre: 'chunk' must have been previously
  //        returned by *this.malloc().
  void ordered_free(void* const chunk) {  //! Same as above, but is order-preserving.
    //!
  ...
  bool is_from(void* const chunk) const {  //! \returns Returns true if chunk was allocated from u or
...
template <typename UserAllocator>
bool pool<UserAllocator>::purge_memory() {  //! pool must be ordered.
...
template <typename UserAllocator>
void* pool<UserAllocator>::malloc_need_resize() {  //! No memory in any of our storages; make a new storage,
...
void* pool<UserAllocator>::
  ...
  if (ptr == 0) {
    if (next_size > 4) {
      next_size >>= 1;
      partition_size = alloc_size();
      POD_size = next_size * partition_size + POD_overhead;
      ptr = (UserAllocator::malloc)(POD_size);
    }
    if (ptr == 0) return 0;
  }
...
template <typename UserAllocator>
void* pool<UserAllocator>::ordered_malloc(
    const size_type n) {  //! Gets address of a chunk n, allocating new memory if not already available.
...
#ifdef BOOST_POOL_INSTRUMENT
  std::cout << "Allocating " << n << " chunks from pool of size " << partition_size << '\n';
#endif
...
class pool {
 public:
  // types
  typedef UserAllocator user_allocator;  // User allocator.
  typedef typename UserAllocator::size_type
      size_type;  // An unsignedegral type that can represent the size of the largest object to be allocated.
  typedef typename UserAllocator::difference_type
      difference_type;  // A signed integral type that can represent the difference of any two pointers.

  // construct/copy/destruct
  explicit pool(const size_type s, const size_type = 32, const size_type m = 0)
      : chunk_size(s), max_alloc_size(m) {}
  ~pool() { purge_memory(); }
  ...
  size_type get_next_size() const { return 1; }
  void set_next_size(const size_type) {}
  size_type get_max_size() const { return max_alloc_size; }
  void set_max_size(const size_type s) { max_alloc_size = s; }
  ...
  void ordered_free(void* const chunk) { return (this->free)(chunk); }
  ...
  void ordered_free(void* const chunk, const size_type n) { (this->free)(chunk, n); }
  bool is_from(void* const chunk) const { return used_list.count(chunk) || free_list.count(chunk); }
