...
struct Cmp3FromLess : public Less {
  ...
  int operator()(V const& a, V const& b) const {
    return Less::operator()(a, b) ? -1 : Less::operator()(b, a);
  }
};
...
template <class Pair>
struct Compare1st {
  bool operator()(Pair const& s1, Pair const& s2) const { return s1.first < s2.first; }
};
...
struct Compare2nd {
  ...
  bool operator()(Pair const& s1, Pair const& s2) const { return s1.second < s2.second; }
};
