...
/** \file
   ...
   bool result=anyv(0);
    ...
    implementation takes address of a template function, so compares semantically equal only in same
   compilation unit

    this is faster than a virtual call. roughly the same as non-virtual member function pointer call

    motivation: to make using AnyGenerator sequences more efficient (calling the AnyVisitor happens
   pre-erasure)
*/
...
/*
  TODO: assuming no performance penalty, use mechanism like sdl::function<bool (int)> to make creation of
...
struct AnyVisitor {
  ...
  AnyVisitor& operator=(AnyVisitor const& o) const {
    pany = o.pany;
    pobj = o.pobj;
  }
  // usage: &free<fnname>
  template <result_type (*Func)(argument_type)>
  static inline result_type free(void*, argument_type a) {
    return (*Func)(a);
  }
