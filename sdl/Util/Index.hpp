...
template <class Ptr>
struct DeleteKey {
  void operator()(Ptr p) const { delete p; }
  template <class Val>
...
struct DeleteIndexVisitor : public Util::IndexVisitor<Ptr> {
  virtual void visit(Ptr const& p) const { delete p; }
};
...
class Index {
 ...
 public:
  ...
  Index() : freelist_((PointerOrFreeIndex)kNullFreelistIndex) {
  ...
  /**
     \return id such that *get(id) == *k,

     \param[in] k has ephemeral item pointer (not deleted in any case! - so you should delete if existing)

     \param[out] existing pointer in index if k was already present, else NULL
  ...
  */
  IdType addOrExisting(Ptr k, Ptr& existing) {
    ...
    if (iNew.second) {
      ...
      existing = iNew.first->first;
      SDL_TRACE(PhraseBased.Index.addIsNew, "rereached state " << *k << " => id=" << iNew.first->second
                                                               << " (setting to previous state " << *existing
                                                               << ')');
  ...
  IdType add(Ptr& k) {
    Ptr existing;
    IdType r = addOrExisting(k, existing);
    if (existing) {
      delete k;
      k = existing;
    }
  ...
  /**
     ...
     \param[inout] k gets pointer to item in index;

     \param[out] isNew means k *is* the (new) pointer. !isNew means k is set to the (existing) pointer in
     index
  */
  ...
  bool contains(Ptr k) const { return indexInData_.find(k) != indexInData_.end(); }

  IdType get(Ptr& k) const {
    typename Map::const_iterator found = indexInData_.find(k);
    if (found == indexInData_.end()) throw IndexException("Util::Index key not found");
    setPtr(k, found->first);
    return found->second;
  }

  IdType getConst(Ptr k) const { return get(k); }
...
// shouldn't be necessary; debugging CM-434 crash in release
    ...
    for (std::size_t b = 0, be = indexInData_.bucket_count(); b < be; ++b) {
      for (typename Map::const_local_iterator i = indexInData_.cbegin(b), end = indexInData_.cend(b);
           i != end;) {
        assert(!lsb(i->first));
#if SDL_INDEX_CAREFUL_DELETE
        *o++ = i->first;
  ...
  void reserveIdsUpTo(IdType id) {
    if (id > data_.size()) data_.resize(id);
  }

  bool empty() { return indexInData_.empty(); }
 ...
 private:
  ...
  enum
#endif
  { kMaxPointerOrFreeIndex = (PointerOrFreeIndex)-1,  // we need to keep this value to force uint64 type (if
    // you have just 0x7ff.... then it's no good - your enum
    // might be signed)
    kNullFreelistIndex = (PointerOrFreeIndex)kMaxPointerOrFreeIndex >> 1 };
  PointerOrFreeIndex freelist_;  // first free index, and next is index(ptrOrFreeList_[freelist_]), or else
  // kNullFreelistIndex
};
