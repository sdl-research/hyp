...
struct SharedGenerator<PtrGen, NonPeekableT>
    : GeneratorBase<SharedGenerator<PtrGen>, typename RemovePointer<PtrGen>::type::result_type, NonPeekableT> {
  typedef typename RemovePointer<PtrGen>::type Generator;
  typedef PtrGen Ptr;
  Ptr p;
  Generator& generator() const { return *p; }
  SharedGenerator(Ptr const& p = PtrGen()) : p(p) {}
  SharedGenerator(SharedGenerator const& other) : p(other.p) {}
  //  SharedGenerator(Generator *newGen) : p(newGen) {}
  typename Generator::result_type operator()() { return (*p)(); }
  operator bool() const { return p && (bool)*p; }
  SharedGenerator clone() const { return p ? SharedGenerator(new Generator(*p)) : SharedGenerator(); }
  friend inline std::ostream& operator<<(std::ostream& out, SharedGenerator<PtrGen, NonPeekableT> const& self) {
    if (self.p)
      out << self.generator();
    else
      out << "(NULL)";
...
struct SharedGenerator<PtrGen, PeekableT>
    : GeneratorBase<SharedGenerator<PtrGen>, typename RemovePointer<PtrGen>::type::result_type, PeekableT> {
  typedef typename RemovePointer<PtrGen>::type Generator;
  typedef PtrGen Ptr;
  Ptr p;
  Generator& generator() const { return *p; }
  SharedGenerator(Ptr const& p = Ptr()) : p(p) {}
  SharedGenerator(SharedGenerator const& other) : p(other.p) {}
  //  SharedGenerator(Generator *newGen) : p(newGen) {}
  typename Generator::result_type peek() const { return p->peek(); }
  ...
  operator bool() const { return p && (bool)*p; }
  SharedGenerator clone() const { return p ? SharedGenerator(new Generator(*p)) : SharedGenerator(); }
  friend inline std::ostream& operator<<(std::ostream& out, SharedGenerator<PtrGen, PeekableT> const& self) {
    if (self.p)
      out << self.generator();
    else
      out << "(NULL)";
