...
/** \file
  ...
  void printVocab(LMimpl &lm)
   ...
   Generator concept (similar to pair of input iterators, but less implementation burden and simpler
  interface):

   Generator g(...); // generators are modified by using them (are lvalues, not iterators/handles)
   typedef Generator::result_type V; // this should be a reference type if the underlying implementation can
  return it. use Generator::value_type if you want to hold the result
   while(g) {
   ...
   struct MyGenerator : GeneratorBase<MyGenerator, int, PeekableT> // inheriting adds typedefs, and gives
  default Generator operator() from peek and pop
   {
   int state;
  ...
  void pop() { --state; }
   ...
   template <class Gen>
  bool peekable(typename std::enable_if<is_void<typename Gen::Peekable>::value,Gen>::type &gen) { return true;
  }

   also:

   std::vector<V> vec;
   int n=appendGenerate(g, vec,10); // like v.push_back(g()) up to 10 times, returning number of items
  appended.
   int n=appendGenerate(g, vec); // unlimited
   sdl::function<bool (V const&)> f;
   int n = visitGenerate(g, f,10); // same as: vec.clear();append(vec, g,10);for (V const& x : vec) if (!f(x))
  break;
...
/*
  ...
  note: iterators will only compare equal when they're done. i.e. for PeekableT g you could use the
  boost::range (g, g)
...
typename std::enable_if<is_non_peekable<Gen>::value, std::size_t>::type appendGenerate(Gen& gen, Vec& vec,
                                                                                       std::size_t max) {
  UTIL_DBG_MSG(12, "non-peekable generate(max)");
  std::size_t n = 0;
  vec.reserve(vec.size() + max);
  for (; n < max && gen; ++n) vec.push_back(gen());
  return n;
}
...
template <class Gen, class Vec>
typename std::enable_if<is_peekable<Gen>::value, std::size_t>::type appendGenerate(Gen& gen, Vec& vec,
                                                                                   std::size_t max) {
...
template <class Gen, class Visit>
typename std::enable_if<is_peekable<Gen>::value, std::size_t>::type visitGenerate(Gen& gen, Visit const& visit,
                                                                                  std::size_t max) {
...
struct GeneratorDesc {
  friend inline std::ostream& operator<<(std::ostream& out, GeneratorDesc const& self) {
    out << "(generator)";
    return out;
  }
};
...
struct GeneratorTraitsBase {
 ...
 public:
  typedef typename boost::remove_const<maybe_const_value_type>::type
      value_type;  // you can always use this to store a copy (e.g. making a nonpeekable generator peekable)
...
// you can a peekable generator as an input iterator range only if you can create a finished "end" of same
// type that compares false. then you're equal to end exactly when you're done too
template <class Derived, class Value>
class GeneratorAsIterator
    : public boost::iterator_facade<Derived, Value, std::input_iterator_tag, Value const&> {
  friend class boost::iterator_core_access;

  void increment() { static_cast<Derived*>(this)->pop(); }

  Value const& dereference() const { return static_cast<Derived const*>(this)->peek(); }
...
template <class Derived, class Value>
struct GeneratorBase<Derived, Value, PeekableT> : GeneratorTraits<Derived, Value, PeekableT>
//, GeneratorAsIterator<Derived, Value>
{
  typedef typename GeneratorBase::value_type value_type;
  value_type
  operator()() {  // values returned by reference may not remain valid for a peekable gen, after you pop.
...
struct NullGenerator : GeneratorBase<NullGenerator<Value>, Value, PeekableT> {
  // cppcheck-suppress nullPointer
  Value const& peek() const {
    assert(0);
    return *(Value const*)0;
  }  // never (correctly) called
  void pop() const {}
  operator bool() const { return false; }
};

template <class Value>
struct UnitGenerator : GeneratorTraits<UnitGenerator<Value>, Value, PeekableT> {
...
struct FuncGenerator : GeneratorBase<FuncGenerator<Func, Result>, Result, NonPeekableT> {
