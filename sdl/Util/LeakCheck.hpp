...
#define SDL_LEAK_CHECK_NAME_GLOBAL(type, name, nonce, global) \
  sdl::Util::LeakCheck<type, global> const SDL_GENSYM(gLeakCheck##nonce)(name);
#define SDL_LEAK_CHECK_NAME_GLOBAL_NS(type, name, nonce, global) \
  namespace {                                                    \
  SDL_LEAK_CHECK_NAME_GLOBAL(type, name, nonce, global)          \
  }
#else
#define SDL_LEAK_CHECK_NAME_GLOBAL(type, name, nonce, global)
#define SDL_LEAK_CHECK_NAME_GLOBAL_NS(type, name, nonce, global)
...
struct LeakCheckBase : ILeakCheck {
  ...
  std::size_t expect;
  char const* name;
  void init(char const* classname = "Object", bool enableCheck = true, bool nonGlobalSoYouCanLog = true) {
    enable = enableCheck;
    log = nonGlobalSoYouCanLog;
    name = classname;
    start();
  }
  LeakCheckBase(bool enable = true) : expect(), name("Object"), enable(enable), log() {}
  bool enable;
  bool log;
  ...
  void start() {
    if (enable) {
      expect = size();
      if (log) SDL_DEBUG(Leak.LeakCheck.start, "initial count of " << *this << ": " << expect);
    }
  }
  void finish() {
    if (enable) {
...
#if SDL_OBJECT_COUNT
  std::size_t size() const { return ObjectCount<Val>::size(); }
  LeakCheck(char const* classname) : LeakCheckBase(!globalScope && Util::isSingleThreadProgram()) {
    name = classname;
    log = !globalScope;
    if (globalScope) enable = true;
    ObjectCount<Val>::setName(name);
    if (SDL_OBJECT_COUNT_REPORT_NONZERO_INIT && expect && globalScope)
      std::cerr << "\nLeakCheck<" << *this << ">: at startup, " << expect
                << " live objects at global scope - so there must be (indirectly) global objects of type '"
                << name << "'\n";
    start();
  }
#else
  LeakCheck(char const* classname) : LeakCheckBase(false) { name = classname; }
#endif
};
