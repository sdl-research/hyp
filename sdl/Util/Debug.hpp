...
#define IFDBGENV(ch, level, x)                                   \
  do {                                                           \
    if (SDL_DBG_MAX_LEVEL >= level && ch##_DBG_LEVEL >= level) { \
      x;                                                         \
    }                                                            \
  } while (0)
...
// decrease this to lose even overhead of if-enabled check; sets max effective level of
// getenv_int("{Channelname}_DBG");
#endif
...
// NOTE: identity macro of same name as debug level is required. I don't know how to automate declaring such
// macros.
...
// Continue on same line (no new header). you can also use for self-contained info msgs you don't want to jump
// to by source line in emacs
