...
/** \file
   ...
   boost::range (g, g)
    ...
    note: peekable anygenerators still return by value. you could use an AnyGenerator<Value const&> or Value *
   instead if you want that efficiency.
*/
...
struct IGeneratorBase : intrusive_refcount<IGeneratorBase<V>, RefCount> {
  // TODO: for hard-to debug reasons, AnyVisitor<V const&> (takes arg by const ref) causes segfault - couldn't
  // get informative line numbers in gdb or valgrind. probably because AnyVisitor::peek returns by value not
  // ref?
...
struct AnyGenerator : AnyGeneratorBase<Value>, GeneratorBase<AnyGenerator<Value, Tag>, Value, Tag> {
...
template <class Value>
struct AnyGenerator<Value, PeekableT> : AnyGeneratorBase<Value>,
