...
struct PerformancePerOptions {
  ...
  void configure(Config& c) {
    ...
    c("name", &name)("name for performance counter (if empty, a reasonable default will apply).")
        .verbose()
        .init("");
  }
  /** Is anything enabled? */
  bool enabled() const { return showTotal || showIncrement; }
  void disable() { showIncrement = showTotal = false; }
  /** Should memory be reported for every input?

      Probably the most heavyweight scenario
  */
  bool everyMem() const { return showIncrement && measureMemory; }
...
struct PerformancePer {
 private:
  ...
  std::string const& inputSizeUnits() const { return totalInputSize.units; }
  Flag reported;

 public:
  static inline std::string prefixSeparator() { return "."; }
  MeasureProcessMemory measuringMemory() const {
    return opt.measureMemory ? kMeasureProcessMemory : kNoMeasureProcessMemory;
  }
  ...
  InputSize sizePerInput() const { return nInputs ? totalInputSize / nInputs : InputSize(); }
  ...
  struct RecordElapsed {
    ...
    ~RecordElapsed() {
      if (ptotal && ptotal->enabled()) ptotal->recordElapsed(inputSize, elapsed());
    }
  };
};
