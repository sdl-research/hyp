...
struct Times {
  ...
  result_type operator()(A const& a, B const& b) const { return a*b; }
};

template <class Result = double>
struct DotProductCombine {
  typedef Result result_type;
  static inline result_type zero() {
    return Result();
  }
  inline void plusBy(result_type const& delta, result_type &sum) const { sum += delta; }
  template <class A, class B>
  inline result_type operator()(A const& a, B const& b) const { return a*b; }
};


//there's no optimization for sorted Map case, nor do i think there should be. if things are that dense, use vectors not maps
...
struct DotProductResult : DotProductCombine<Result> {
  ...
  template <class A, class B>
  inline void operator()(A const& a, B const& b) {
...
void selfIntersectValues(PairIter i, PairIter end, DotProductResult &dotResult) {
  for (; i!=end; ++i)
    dotResult(i->second, i->second); //TODO: iterator_traits value_type second_type?
}
