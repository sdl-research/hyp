...
/** \file
   ...
   to sdl::Util, and also adding our own dynamic_bitset_fwd.hpp (i.e. this
   coexists w/ the official boost::dynamic_bitset)

   it's possible that some externally specialized things (serialization?) will
   be less optimized unless we set them ourselves. i took this approach instead
   of maintaining a customized boost header in externals.


   added rfind_first, rfind_next

   added test_set test_reset test_flip

   added block (read-only) access

   (unit tests added for those, passed - run 'bjam' in dir

   https://github.com/graehl/boost/tree/dynamic_bitset/libs/dynamic_bitset

   )

   since this header hasn't changed in a long time in boost (not at all from
   some time before 1.49 to 1.55) there's no urgency to get this merged, but I'd
   like it to go into official boost at some point; otherwise we have to check
...
/// after this line, follows github version except for incl. guard + namespace
...
struct BitsSet {
  enum { kUseTable = true };
  static inline Int builtinCountBits(Int i) {
    return 0;
  }
};

template <>
struct BitsSet<std::size_t> {
  typedef std::size_t Int;
  enum { kUseTable = false };
  static inline Int builtinCountBits(Int i) {
    return __builtin_popcountl(i);
  }
};

# if UINT64_DIFFERENT_FROM_SIZE_T
template <>
struct BitsSet<boost::uint64_t> {
  typedef boost::uint64_t Int;
  enum { kUseTable = false };
  static inline Int builtinCountBits(Int i) {
    return __builtin_popcountl(i);
  }
...
inline std::size_t do_count_builtin(
    Iterator first, std::size_t length, ValueType v,
    boost::detail::dynamic_bitset_impl::value_to_type<boost::detail::dynamic_bitset_impl::access_by_blocks>* vt)
{
# if __GNUC__ >= 4
  typedef BitsSet<ValueType> BS;
  if (!BS::kUseTable) {
    std::size_t num = 0;
    while (length) {
      num += builtinCountBits(*first);
      ++first;
      --length;
    }
    return num;
...
# endif
    return boost::detail::dynamic_bitset_impl::do_count(first, length, v, vt);
}

template <typename Iterator, typename ValueType>
inline std::size_t do_count_builtin(
    Iterator first, std::size_t length, ValueType v,
    boost::detail::dynamic_bitset_impl::value_to_type<boost::detail::dynamic_bitset_impl::access_by_bytes>* vt)
{
    return boost::detail::dynamic_bitset_impl::do_count(first, length, v, vt);
}
...
class dynamic_bitset
{
  ...
  BOOST_STATIC_ASSERT((bool)boost::detail::dynamic_bitset_impl::allowed_block_type<Block>::value);
...
public:
    ...
    class reference
    {
        ...
        reference(block_type & b, block_type pos)
            :block_(b),
             mask_( (assert(pos < bits_per_block),
                      block_type(1) << pos )
                   )
        { }
    ...
    public:
        ...
        reference& operator=(reference const& rhs) { do_assign(rhs); return *this; } // for b[i] = b[j]
     ...
     private:
        ...
        void do_set() { block_ |= mask_; }
        void do_reset() { block_ &= ~mask_; }
        void do_flip() { block_ ^= mask_; }
    ...
    // constructors, etc.
    explicit
    dynamic_bitset(Allocator const& alloc = Allocator());
    ...
    void dispatch_init(T num_bits, unsigned long value,
                       boost::detail::dynamic_bitset_impl::value_to_type<true>)
    {
    ...
    void dispatch_init(T first, T last,
                       boost::detail::dynamic_bitset_impl::value_to_type<false>)
    {
    ...
    template <typename BlockInputIterator>
    void append_(BlockInputIterator first, BlockInputIterator last, std::input_iterator_tag)
    {
        std::vector<Block, Allocator> v(first, last);
        append_(v.begin(), v.end(), std::random_access_iterator_tag());
    }
    template <typename BlockInputIterator>
    void append_(BlockInputIterator first, BlockInputIterator last, std::forward_iterator_tag)
    {
        assert(first != last);
        ...
        num_bits_ += bits_per_block * d;
    }
    template <typename BlockInputIterator>
    void append(BlockInputIterator first, BlockInputIterator last) // strong guarantee
    {
        if (first != last) {
            typename boost::detail::iterator_traits<BlockInputIterator>::iterator_category cat;
            append_(first, last, cat);
        }
    }
    ...
    bool is_subset_of(dynamic_bitset const& a) const;
    bool is_proper_subset_of(dynamic_bitset const& a) const;
    bool intersects(dynamic_bitset const& a) const;
    ...
    size_type rfind_next(size_type pos) const;

    block_type block(size_type block_index) const {
      assert(block_index < bits_.size());
      return bits_[block_index];
    }

    block_type& block(size_type block_index) {
      assert(block_index < bits_.size());
      return bits_[block_index];
    }
...
private:
    ...
    void zero_unused_bits_();
    bool check_invariants_() const;

    size_type do_find_from_(size_type first_block) const;
    size_type do_rfind_from_(size_type first_block) const;
                                 ...
                                 Allocator const& alloc*/)
    {
        ...
        bits_.resize(calc_num_blocks(num_bits));
        num_bits_ = num_bits;
        ...
        typename buffer_type::iterator it = bits_.begin();
...
BOOST_DYNAMIC_BITSET_PRIVATE:
    ...
    bool unchecked_test_(size_type pos) const;

    static size_type calc_num_blocks(size_type num_bits);

    Block&        highest_block_();
    Block const&  highest_block_() const;
    ...
    class bit_appender {
      ...
      // not implemented
      bit_appender(bit_appender const&);
      bit_appender & operator=(bit_appender const&);
...
template <typename Block, typename Allocator>
dynamic_bitset<Block, Allocator>::
dynamic_bitset(size_type num_bits, unsigned long value, Allocator const& alloc)
    : bits_(alloc),
      num_bits_(0)
{
...
dynamic_bitset(dynamic_bitset const& b)
  : bits_(b.bits_), num_bits_(b.num_bits_)
{
...
resize(size_type num_bits, bool value) // strong guarantee
{
  ...
  //    except a call to zero_unused_bits_()
  //
...
template <typename Block, typename Allocator>
dynamic_bitset<Block, Allocator>&
dynamic_bitset<Block, Allocator>::operator&=(dynamic_bitset const& rhs)
{
...
dynamic_bitset<Block, Allocator>::operator<<=(size_type n)
{
    ...
    if (n > 0) {
        ...
        zero_unused_bits_(); // thanks to Lester Gong
...
template <typename Block, typename Allocator>
bool dynamic_bitset<Block, Allocator>::unchecked_test_(size_type pos) const
{
    return (bits_[block_index(pos)] & bit_mask(pos)) != 0;
}

// set(pos) and return previous bit at pos
template <typename Block, typename Allocator>
bool dynamic_bitset<Block, Allocator>::test_set(size_type pos)
{
    assert(pos < num_bits_);
    assert(block_index(pos) < bits_.size());
    Block &modify_block = bits_[block_index(pos)];
    Block mask = bit_mask(pos);
    bool bit = modify_block & mask;
    modify_block |= mask;
    return bit;
}

// reset(pos) and return previous bit at pos
template <typename Block, typename Allocator>
bool dynamic_bitset<Block, Allocator>::test_reset(size_type pos)
{
    assert(pos < num_bits_);
    Block &modify_block = bits_[block_index(pos)];
    Block mask = bit_mask(pos);
...
// flip(pos) and return previous bit at pos
template <typename Block, typename Allocator>
bool dynamic_bitset<Block, Allocator>::test_flip(size_type pos)
{
    assert(pos < num_bits_);
    Block &modify_block = bits_[block_index(pos)];
    Block mask = bit_mask(pos);
...
{
    using boost::detail::dynamic_bitset_impl::table_width;
    using boost::detail::dynamic_bitset_impl::access_by_bytes;
    using boost::detail::dynamic_bitset_impl::access_by_blocks;
    using boost::detail::dynamic_bitset_impl::value_to_type;
    ...
    return do_count_builtin(bits_.begin(), num_blocks(), Block(0),
...
dynamic_bitset<Block, Allocator>::max_size() const
{
    ...
    const size_type m = boost::detail::dynamic_bitset_impl::
                        vector_max_size_workaround(bits_);
...
template <typename Block, typename Allocator>
bool dynamic_bitset<Block, Allocator>::intersects(dynamic_bitset const& b) const
{
...
template <typename Block, typename Allocator>
typename dynamic_bitset<Block, Allocator>::size_type
dynamic_bitset<Block, Allocator>::do_find_from_(size_type first_block) const
{
    size_type i = first_block;

    // skip null blocks
    while (i < num_blocks() && bits_[i] == 0)
        ++i;

    if (i >= num_blocks())
        return npos; // not found

    return i * bits_per_block + boost::lowest_bit(bits_[i]);

}


// reverse order version of do_find_from_: look for the last bit "on", starting
// from the block with index first_block, down to block index 0 (returning npos
// if none found)
//
template <typename Block, typename Allocator>
typename dynamic_bitset<Block, Allocator>::size_type
dynamic_bitset<Block, Allocator>::do_rfind_from_(size_type first_block) const
{
  assert(first_block != npos);
  ...
  return i * bits_per_block + boost::integer_log2(bits_[i]); // integer log2 = position of highest set bit
}
...
{
    return do_find_from_(0);
}

// look for the last bit "on"
template <typename Block, typename Allocator>
typename dynamic_bitset<Block, Allocator>::size_type
dynamic_bitset<Block, Allocator>::rfind_first() const
{
  return bits_.empty() ? npos : do_rfind_from_(bits_.size() - 1);
}
...
dynamic_bitset<Block, Allocator>::find_next(size_type pos) const
{
    ...
    const Block fore = bits_[blk] >> ind;

    return fore ?
        pos + boost::lowest_bit(fore)
        :
        do_find_from_(blk + 1);
...
template <typename Block, typename Allocator>
typename dynamic_bitset<Block, Allocator>::size_type
dynamic_bitset<Block, Allocator>::rfind_next(size_type pos) const
{
  assert(pos != npos);
  if (pos == 0)
    return npos;
           ...
           const dynamic_bitset<Block, Alloc>& b)
{
    ...
    class pseudo_sentry {
        ...
        const bool ok_;
    public:
        explicit pseudo_sentry(std::istream & r) : r_(r), ok_(r.ipfx(0)) { }
        ~pseudo_sentry() { r_.isfx(); }
...
//
template <typename Block, typename Allocator>
inline Block& dynamic_bitset<Block, Allocator>::highest_block_()
{
    return const_cast<Block &>
           (static_cast<const dynamic_bitset *>(this)->highest_block_());
}

// gives a const-reference to the highest block
//
template <typename Block, typename Allocator>
inline Block const& dynamic_bitset<Block, Allocator>::highest_block_() const
{
...
//
template <typename Block, typename Allocator>
inline void dynamic_bitset<Block, Allocator>::zero_unused_bits_()
{
    assert (num_blocks() == calc_num_blocks(num_bits_));

    // if != 0 this is the number of bits used in the last block
    const block_width_type extra_bits = count_extra_bits();

    if (extra_bits != 0)
        highest_block_() &= ~(~static_cast<Block>(0) << extra_bits);

}

// check class invariants
template <typename Block, typename Allocator>
bool dynamic_bitset<Block, Allocator>::check_invariants_() const
{
    const block_width_type extra_bits = count_extra_bits();
    if (extra_bits > 0) {
        block_type const mask = (~static_cast<Block>(0) << extra_bits);
        if ((highest_block_() & mask) != 0)
            return false;
    }
    if (bits_.size() > bits_.capacity() || num_blocks() != calc_num_blocks(size()))
        return false;
