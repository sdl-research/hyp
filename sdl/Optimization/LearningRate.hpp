...
class ConstantLearningRateFct : public ILearningRate {
 public:
  ConstantLearningRateFct(SdlFloat eta_ = floatCast(0.1))
  : eta(eta_) {}
...
class ExponentialLearningRateFct : public ILearningRate {
 public:
  ...
  void setNumUpdates(std::size_t num) {
    numUpdates = (SdlFloat)num;
  }

  SdlFloat operator()(std::size_t iter) {
    return initialEta * pow(alpha, iter / numUpdates);
  }
...
struct LearningRateOptions {
  ...
  void configure(Config& config) {
    ...
    config("adagrad-rate", &adagradRate).init(1.0f)
        ("Adagrad rate (eta). Initial rate for each feature, which decreases whenever feature is observed.");
    config("adagrad-l1-strength", &adagradL1Strength).init(0.0f)
        ("Adagrad L1 strength (lambda). Positive number; 0 means no L1.");
  }
