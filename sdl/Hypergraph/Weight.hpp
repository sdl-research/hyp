...
//TODO: #if HAVE_OPENFST and use constants appropriate for particular semiring - this is just to compile HgFsmDraw
...
class FloatWeightTpl : public WeightBase {
 ...
 public:
  ...
  // TODO: would prefer default value_ is 0 for consistency; make that change and test.
  FloatWeightTpl() : value_(std::numeric_limits<T>::infinity()) {}
  ...
  void setValue(DoubleT v) {
    value_ = (T)v;
  }
  void setValue(int v) {
    value_ = (T)v;
  }
  void setValue(std::size_t v) {
    value_ = (T)v;
  }
...
template <class T> std::istream & operator>>(std::istream &i, FloatWeightTpl<T> &x) { return i>>x.value(); }
template <class T> std::ostream & operator<<(std::ostream &o, FloatWeightTpl<T> const& x) { return o << x.getValue(); }
...
template<class T>
inline bool approxGreaterOrEqual(const FloatWeightTpl<T>& w1,
                                 const FloatWeightTpl<T>& w2,
                                 T epsilon = FloatConstants<T>::epsilon) {
...
template<class T>
inline bool approxLessOrEqual(const FloatWeightTpl<T>& w1,
                              const FloatWeightTpl<T>& w2,
                              T epsilon = FloatConstants<T>::epsilon) {
...
template<class T>
inline bool definitelyGreater(const FloatWeightTpl<T>& w1,
                              const FloatWeightTpl<T>& w2,
                              T epsilon = FloatConstants<T>::epsilon) {
...
template<class T>
inline bool definitelyLess(const FloatWeightTpl<T>& w1,
                           const FloatWeightTpl<T>& w2,
                           T epsilon = FloatConstants<T>::epsilon) {
  return Util::definitelyLess(w1.getValue(), w2.getValue(), epsilon);
}

// all 3 of approx equal, greater, less may be true simultaneously (if same eps, then approxEqual <=> approxGreater and approxLess
...
class BooleanWeight : public FloatWeightTpl<bool> {
 public:
  ...
  static inline BooleanWeight one() {
    return BooleanWeight(true);
  }

  typedef void HasIsZero;
  bool isZero() const { return value_==false; }
  static inline BooleanWeight zero() {
...
class ViterbiWeightTpl : public FloatWeightTpl<T> {
 ...
 public:
  ...
  static inline ViterbiWeightTpl<T> one() {
    return ViterbiWeightTpl<T>(0.0f);
  }

  static inline ViterbiWeightTpl<T> zero() {
    return ViterbiWeightTpl<T>(FloatLimits<T>::posInfinity);
  }
...
class LogWeightTpl : public FloatWeightTpl<T> {
 ...
 public:
  ...
  static inline LogWeightTpl<T> one() {
    return LogWeightTpl<T>(0.0f);
  }

  static inline LogWeightTpl<T> zero() {
  ...
  void timesBy(LogWeightTpl const& b) {
    this->value_ += b.value_;
  }
  typedef void HasTimesBy;
...
inline char const* weightName(BooleanWeight *) { return "Boolean"; }
