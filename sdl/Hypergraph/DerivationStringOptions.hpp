...
struct WhichSymbolOptions {
  ...
  void configure(Config& config, bool hide = false) {
    ...
    config("block", &block)
        .verbose(hide)
        .defaulted()("skip or keep block/entity symbols (<xmt-blockN> </xmt-block> <xmt-entityN>)");
    config("label-type", &labelType)
        .verbose(hide)
        .init(kOutput)
        .verbose()(
            "Print input or output for transducer state labels, where output may differ from input. 'output' "
            "will use the input symbol if the state has no output label.");
    config("leaf-only", &leafOnly)
        .verbose(hide)
        .init(kLeafOnly)(
            "(false, or whole-tree, means print all derivation states' nodes' labels) (true, or leaf-only; "
            "means "
...
struct DerivationStringOptions : WhichSymbolOptions {
  ...
  void configure(Config& config, bool hide = false) {
    ...
    config("space", &space)
        .verbose(hide)
        .defaulted()("separate tokens with this (always a space between weight and tokens)");
  }
};
