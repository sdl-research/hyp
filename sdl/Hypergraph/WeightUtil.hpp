...
struct PlusBy {
  typedef W By;
  typedef W Result;
  static inline void plusBy(By const& b, Result& a) { a = Hypergraph::plus(a, b); }
  /**
     for use with e.g. Map.hpp updateBy(PlusBy<W>(), map, key, W(delta)).
  */
  void operator()(By const& b, Result& a) const { a = Hypergraph::plus(a, b); }
};
...
struct PlusBy<W, typename W::HasPlusBy> {
  typedef W By;
  typedef W Result;
  static inline void plusBy(By const& b, Result& a) { a.plusBy(b); }
  void operator()(By const& b, Result& a) const { a.plusBy(b); }
};
...
struct TimesBy {
  typedef W By;
  typedef W Result;
  static inline void timesBy(By const& b, Result& a) { a = Hypergraph::times(a, b); }
};
...
struct TimesBy<W, typename W::HasTimesBy> {
  typedef W By;
  typedef W Result;
  static inline void timesBy(By const& b, Result& a) { a.timesBy(b); }
};
...
// note: minus, divide, multiplicative inverse, power - will throw (unsupported operation) for general
// semirings. only some of our weight classes happen to offer these
...
template <class T>
inline bool checkPlusBy(ViterbiWeightTpl<T> const& a, ViterbiWeightTpl<T> const& b,
...
template <class T>
inline bool checkPlusBy(FloatWeightTpl<T> const& a, FloatWeightTpl<T> const& b,
                        T epsilon = FloatConstants<T>::epsilon) {
  return approxLessOrEqual(a, b, epsilon);  // this would need to change if we had negative LogWeight
}

inline bool plusByChanges(BooleanWeight const& a, BooleanWeight const& b) {
  return a.getValue() < b.getValue();
}

inline bool plusByChanged(BooleanWeight const& a, BooleanWeight& b) {
  if (a.getValue() < b.getValue()) {
    b = a;
    return true;
  }
...
template <class T>
struct ProbToNeglog {
  FloatWeightTpl<T> operator()(T prob) const { return FloatWeightTpl<T>(-std::log(prob)); }
};

template <class T>
struct Identity {
  FloatWeightTpl<T> operator()(T prob) const { return prob; }
};
