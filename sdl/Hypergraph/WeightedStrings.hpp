...
/// you can build up the set one sequence at a time with either addString() or this protocol: openString(Weight), addChar(SymbolId) repeatedly, closeString().
template<class W>
struct WeightedStrings {
  ...
  Sym charId(std::string const& s) const {
    return lexicalSymbol(s, *voc);
  }
  ...
  void print(std::ostream &o, Syms const& s) const {
    Util::print(o, s, stateRange(voc, sepchars));
  }
  ...
  friend inline void print(std::ostream &o, std::size_t i, WeightedStrings<W> const& ws) {
    ws.print(o, i);
  }
