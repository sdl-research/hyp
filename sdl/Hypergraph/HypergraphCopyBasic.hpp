...
struct ArcCopyPreserveStateIdsFct {
  ArcCopyPreserveStateIdsFct(IMutableHypergraph<Arc>* pTarget) : pTarget_(pTarget) {}
  void operator()(ArcBase const* arc) const { pTarget_->addArc(new Arc(*(Arc*)arc)); }
...
struct ArcCopySome : public Keep {
  IMutableHypergraph<Arc>* pTarget_;
  ArcCopySome(IMutableHypergraph<Arc>* pTarget, Keep const& r) : pTarget_(pTarget), Keep(r) {}
  void operator()(Arc const& arc) const {
    if (Keep::operator()(arc)) pTarget_->addArc(new Arc(arc));
  }
  void operator()(ArcBase const* arc) const { (*this)(*(Arc*)arc); }
};
...
enum ClearAndSameProperties { kNoClear = 0, kClearAndSameProperties = 1 };

template <class A, class B>
void copyExceptArcs(IHypergraph<A> const& from, IMutableHypergraph<B>* to,
  ...
  if (from.properties() & kSortedStates) to->setSortedStatesNumNotTerminal(from.exactSizeForHeads());
}
...
void copyHypergraphRelabel(IHypergraph<Arc> const& from, IMutableHypergraph<Arc>* to, T const& tr) {
...
*/
template <class Arc>
void copyHypergraph(IHypergraph<Arc> const& from, IMutableHypergraph<Arc>* to,
                    ClearAndSameProperties clearAndSameProperties = kClearAndSameProperties) {
...
MutableHypergraph<Arc>* copyHypergraphNew(IHypergraph<Arc> const& from,
                                          shared_ptr<IHypergraph<Arc> const>& holdNew, Properties prop) {
...
*/
template <class Arc, class Keep>
void copyHypergraphPartial(IHypergraph<Arc> const& from, IMutableHypergraph<Arc>* to, Keep const& r,
                           ClearAndSameProperties clearAndSameProperties = kClearAndSameProperties) {
  copyExceptArcs(from, to, clearAndSameProperties);
...
void copyHypergraphRelabelPartial(IHypergraph<Arc> const& from, IMutableHypergraph<Arc>* to, T const& tr,
                                  Keep const& r) {
  copyStatesRelabel(from, to, tr);
  from.forArcs(copyArcs(to, r));
}
...
template <class A>
void copyEnsuringProperties(IHypergraph<A> const& i, IMutableHypergraph<A>* o, Properties forceOn = 0,
                            Properties forceOff = 0) {
...
*/
template <class A>
IHypergraph<A> const& ensureProperties(IHypergraph<A> const& i, shared_ptr<IHypergraph<A> const>& pMaybeCopyHg,
                                       Properties forceOn = 0, Properties maybeOff = 0, Properties forceOff = 0,
  ...
  if (forceOn & forceOff)
    SDL_THROW_LOG(Hypergraph.HypergraphCopyBasic, std::logic_error,
                  "ensureProperties conflict: forceOn=" << forceOn << " forceOff=" << forceOff
                                                        << " overlap=" << (forceOn & forceOff));
  ...
  if ((switchOn && orSwitchOn) || switchOff) {
    if (onMissing & kMissingThrowMask)
      SDL_THROW_LOG(Hypergraph.HypergraphCopyBasic, HypergraphPropertiesException,
                    "copy not requested for input with properties" << PrintProperties(switchOn)
                                                                   << " missing and with unwanted properties "
                                                                   << PrintProperties(switchOff) << ".");
...
IHypergraph<A>& ensureProperties(IHypergraph<A>& i, shared_ptr<IHypergraph<A>>& p, Properties forceOn = 0,
  ...
  if ((switchOn && orSwitchOn) || switchOff) {
    if (i.isMutable() && onMissing | kMissingModifyMask) {
      ...
      if (onMissing & kMissingThrowMask)
        SDL_THROW_LOG(Hypergraph.HypergraphCopyBasic, HypergraphPropertiesException,
                      "copy not requested for input with properties"
                          << PrintProperties(switchOn) << " missing and with unwanted properties "
                          << PrintProperties(switchOff) << ".");
      MutableHypergraph<A>* pm = new MutableHypergraph<A>((ip | switchOn) & ~switchOff & ~maybeOff);
      p.reset(pm);
      copyHypergraph(i, pm, kNoClear);
...
template <class A>
shared_ptr<IHypergraph<A> const> ensureProperties(IHypergraph<A> const& i, Properties forceOn = 0,
                                                  Properties maybeOff = 0, Properties forceOff = 0,
                                                  OnMissingProperties onMissing = kCopyEnsuringProperties,
                                                  Properties orForceOn = (Properties)-1) {
...
template <class A>
shared_ptr<IHypergraph<A> const> ensureFirstTailOutArcs(IHypergraph<A> const& hg) {
  return ensureProperties(hg, hg.storesOutArcs() ? 0 : kStoreFirstTailOutArcs, kStoreInArcs);
  // also acceptable if hg had kStoreOutArcs
}

template <class A>
shared_ptr<IHypergraph<A> const> ensureOutArcs(IHypergraph<A> const& hg) {
  return ensureProperties(hg, kStoreOutArcs, kStoreInArcs | kStoreFirstTailOutArcs);
}
...
/// deleting shared_ptr
template <class A>
shared_ptr<IHypergraph<A> const> ensureProperties(IHypergraph<A>& i, Properties forceOn = 0,
                                                  Properties maybeOff = 0, Properties forceOff = 0,
                                                  OnMissingProperties onMissing = kCopyEnsuringProperties,
                                                  Properties orForceOn = (Properties)-1) {
