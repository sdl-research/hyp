...
void FeatureIdRange::requireSize(double needed, char const* prefix) {
  ...
  if (n < needed - epsilon)
    SDL_THROW_LOG(Context, ConfigException, prefix << " " << needed << " ids; you allowed only " << n
                                                   << " in " << *this);
...
double FeatureIdRange::tupleRequiresDouble(unsigned order) {
  double n = (double)size();
  double sum = 0;
  for (unsigned len = 2; len <= order; ++len) sum += std::pow(n, (double)len);
  return sum;
}
...
FeatureId FeatureIdRange::tupleRequires(unsigned order) {
  ...
  if (r >= maxFid)
    SDL_THROW_LOG(Context, ConfigException, "tuple order " << order << " for context " << *this
                                                           << " would result in too many features (" << r << ")");
