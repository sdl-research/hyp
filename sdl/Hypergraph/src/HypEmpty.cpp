...
struct HypEmptyOptions {
  ...
  void configure(Config& config) {
    ...
    config("empty", &empty).init(true)("output a line EMPTY or NONEMPTY");
    config("fsm", &fsm).init(false)("output a line FSM or NONFSM");
    config("graph", &graph).init(false)("output a line GRAPH or NONGRAPH");
  }
};
...
struct HypEmpty : TransformMain<HypEmpty> {  // note base class CRTP (google it)
  ...
  void declare_configurable() { this->configurable(&empty); }
  ...
  bool inputTransformInplace(IHypergraph<Arc> const& hg, int) {
    if (empty.empty) outProperty("EMPTY", Hypergraph::empty(hg));
    if (empty.fsm) outProperty("FSM", hg.isFsm());
    if (empty.graph) outProperty("GRAPH", hg.isGraph());
    return true;
  }
};
}
