...
template <class A>
struct Derivation
    : public graehl::shared_nary_tree<Derivation<A>, Util::RefCount>
{
  ...
  Arc * a; // axioms have a==0 so we can keep same tail size/ids instead of skipping them. note that right now they're not labeled w/ stateid; you can get that from their parent - exception: if axiom is final state of hg. oops. so traversals take parent state as arg for now
  ...
  struct ComputeOnceBase
  {
    ...
    /// note: !d.axiom()
    bool open(result_type &, Deriv const&) const
    {
      return true;
    }
    ...
    /// since we use a vector to hold the result_type for a state, you need to use non-default values if you don't want to revisit the same subtree repeatedly. return true always if you don't mind
  ...
  template <class V>
  typename V::result_type computeOnceDfsOneBest(V &v, StateId state, StateId reserveStates = 1000) // pass actual # of states if you like, or max used in deriv + 1
  {
  ...
  friend inline Weight weight(DerivP const& deriv) {
    return deriv->weight();
  }

  friend inline FeatureValue cost(Derivation const& deriv) {
    return deriv.cost();
  }
  friend inline FeatureValue cost(DerivP const& deriv) {
    return deriv->cost();
  }
  ...
  struct VisitOut : public VisitTreeBase
  {
    ...
    {
      bool add = whichStates==kWholeTree ||
          a&&whichStates==kNonLeafOnly ||
    ...
    void close(StateId head, Deriv const& d)
    {
      out(head, d.a);
    }
  };
  ...
  static VisitOut<Util::Adder<Container> const&> visitAdd(Container &o, WhichDerivationStates whichStates = kLeafOnly)
  {
  ...
  struct VisitArcsOut : public VisitTreeBase
  {
    ...
    void close(StateId, Deriv const& d) const
    {
      out(d.a);
    }
  };
  ...
  void appendArcs(std::vector<Arc *> &ret)
  {
    visitTree(visitArcsAdd(ret));
  }
  ...
  */
  ...
  template <class Container>
  void yieldAdd(Container &cont, StateId head, WhichDerivationStates whichStates = kLeafOnly) // note: start state will appear in yield
  {
  ...
  StateString yieldStates(StateId head, WhichDerivationStates whichStates = kLeafOnly)
  {
    return statesFrom(yield(head, whichStates));
  }
  ...
  StateId headUnlessAxiom(StateId elseState = kNoState) const
  {
    return a ? a->head() : elseState;
  }
  ...
  struct TreeVisitItem
  {
    ...
    bool isPop() const
    {
      return pushChild==(TailId)-1;
    }
    void setPop()
    {
      pushChild = (TailId)-1;
    }
    TreeVisitItem(StateId head, Derivation const* dp, TailId pushChild) : head(head), dp(dp), pushChild(pushChild) {}
    ...
    { o << "TreeVisitItem@0x" << (void*)selfp << ": "; if (selfp) selfp->print(o); return o; }
  };
  ...
  struct TreeVisit : private CycleGuard
  {
    ...
    V v;
    Agenda agenda;
    TreeVisit(V v, Derivation const* d, StateId head = kNoState) : v(v)
    {
      this->prepare(*d);
  ...
  typedef Derivation const* DP; // pun: LSB 1 means it's a "close" instruction. safe because sizeof(Derivation)>1.
  ...
  struct DfsVisit
  {
    ...
    void processAgenda()
    {
      ...
      agenda.pop();
      if (Util::lsb(dp))
        return v.close(*Util::withoutLsb(dp));
  ...
  template <class Out>
  void print(Out &o, H const& h, unsigned levels = (unsigned)-1) const {
  ...
  struct VisitPrint : public VisitDfsBase
  {
    ...
    H const& h;
    unsigned levels;
    VisitPrint(Out &o, H const& h, unsigned levels = (unsigned)-1) : o(o), h(h), levels(levels) {}
  ...
  template <class Out>
  friend Out& print(Out &o, self_type const& self, H const& h) { self.print(o, h); return o; }
  template <class Out>
  friend Out& print(Out &o, child_type const& c, H const& h) { c->print(o, h); return o; }
  template <class Out>
  friend Out& print(Out &o, self_type const& self, unsigned levels) { self.print(o, levels); return o; }
  template <class Out>
  friend Out& print(Out &o, child_type const& c, unsigned levels) { c->print(o, levels); return o; }
  ...
  void translateToHypergraph(IHypergraph<A> const& i, IMutableHypergraph<A> &o, StateIdMappingPtr const& idmap, bool best = kIsNonBest)
  {
  ...
  IMutableHypergraph<A> * translateToHypergraphNew(IHypergraph<A> const& i, bool keepStateIds = kRenumberStates, bool best = kIsNonBest)
  {
    IMutableHypergraph<A> *pOut = new MutableHypergraph<A>();
    translateToHypergraph(i, *pOut, keepStateIds, best);
    return pOut;
  }
  shared_ptr<IMutableHypergraph<A> > translateToHypergraph(IHypergraph<A> const& i, bool keepStateIds = kRenumberStates, bool best = kIsNonBest)
  {
    return shared_ptr<IMutableHypergraph<A> >(translateToHypergraphNew(i, keepStateIds, best));
  }

  //TODO: specify output vocab != input vocab?
  void translateToHypergraph(IHypergraph<A> const& i, IMutableHypergraph<A> &o, bool keepStateIds = kRenumberStates, bool best = kIsNonBest)
  {
    if (!best)
      translateToSingleDerivationHypergraph(i, o);
    else {
      StateIdMappingPtr idmap(keepStateIds ?
                              static_cast<StateIdMapping *>(new StateAddIdentityMapping<A>(&o)) :
                              static_cast<StateIdMapping *>(new StateAddMapping<A>(&o)));
      translateToHypergraph(i, o, idmap, best);
    }
  }

  void translateToHypergraph(StateIdTranslation &sx, IMutableHypergraph<A> &o, bool best = kIsNonBest, bool resetColor = true)
  {
    ToHypergraphOnce v(sx, o);
    visitDfs(v);
  }

  // ok for same-head different-subderivation. must pass final state in case final = axiom to get stateid/label right
  template <class V>
  ...
  template <class V>
  typename V::result_type computeOnceDfs(unordered_map<Derivation const*, typename V::result_type> &once, V &v, StateId state) const
  {
    typedef typename V::result_type R;
    if (axiom())
      return v.axiom(state); // NOTE: derivation axioms are placeholders and equal by pointer. so every time you recompute (but don't store in once)
    R *pr;
  ...
  */
  ...
  struct ToSingleDerivationHypergraph : ComputeOnceBase<NewStateAndInArc>
  {
    ...
    NewStateAndInArc axiom(StateId iSt) {
      ...
      if (axiomSt == kNoState)
        axiomSt = o.addState(ihg.labelPair(iSt));
    ...
    ToSingleDerivationHypergraph(IHypergraph<Arc> const& ihg, IMutableHypergraph<Arc> &o)
        : ihg(ihg)
        , o(o)
        , forAxiom(ihg.size(), kNoState)
    {}

    bool open(NewStateAndInArc &r, Deriv const& d) {
      Arc const& arc = d.arc();
      r.second = new Arc(arc, r.first = o.addState()); // sets new head; tails to be overwritten in childClose
  ...
  */
  void translateToSingleDerivationHypergraph(IHypergraph<A> const& i, IMutableHypergraph<A> &o, Properties oProperties = 0)
  {
    StateId iRoot = axiom() ? i.final() : arc().head();
    if (iRoot == kNoState)
      SDL_THROW_LOG(Hypergraph.Derivation, EmptySetException, "can't build derivation hg for singleton derivation and input hg with no final state");
    ...
    //TODO: option for fsm only to produce topo-sorted states (nonlex last, start first, final last). can also do topo sort for hg, but may not want axioms last? for now we just have state 0 final to not break regression output
...
// watch out: entire derivation = kAxiom implies a start=final=axiom toHypergraph i.e. containing empty string only w/ wt 1. in this case the state id is irrelevant - make it up
template <class A>
typename Derivation<A>::DerivP Derivation<A>::kAxiom = Derivation<A>::construct(0); //0 arc pointer - to get state you need to look at tails of parent.
...
typename Derivation<Arc>::DerivP singleDerivation(IHypergraph<Arc> const& hg, StateSet &seen, StateId from)
{
  ...
  for (TailId i = 0; i<n; ++i)
    ch[i] = singleDerivation(hg, seen, tails[i]);
...
singleDerivationFsmOut(IHypergraph<Arc> const& hg, StateSet &seen
                       , typename Derivation<Arc>::DerivP const& bottomUpPart, StateId from, StateId final)
{
  //assert(hg.isFsm())
  assert(from!=kNoState);
  ...
  if (!Util::latch(seen, from))
    SDL_THROW2(CycleException, "Cycle observed - state appeared twice in stack:", from);
  ArcIdRange r = hg.outArcIds(from);
  std::size_t nr = boost::size(r);
  if (nr!=1) {
    if (nr>1)
  ...
  ArcId aid = *boost::begin(r);
  Arc* arc = hg.outArc(from, aid);
  if (!arc->isFsmArc())
    SDL_THROW(NonFsmHypergraphException, Util::print(arc, hg));
  Child ret = Deriv::construct(arc, 2);
...
typename Derivation<Arc>::DerivP singleDerivation(IHypergraph<Arc> const& hg)
{
  try {
    ...
    if (hg.storesOutArcs() && hg.isFsm()) {
      ...
      return singleDerivationFsmOut(hg, seen, Deriv::kAxiom, s, hg.final());
...
{
  return typename Derivation<Arc>::DerivAndWeight(p,
...
struct TreeVisitArcs {
  ...
  explicit TreeVisitArcs(ArcVisitor const& visitor)
      : visitor(visitor)
  {}
  template <class Arc>
  bool open(StateId, Derivation<Arc> const&) const { return true; }
  template <class Arc>
