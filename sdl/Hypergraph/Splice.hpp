...
{
  //TODO@JG: a separate prefixing transform (in place) is just as efficient and the optimization described above could be implemented more generally (recognizing the structure of "start (single arc) -> rest of fsm""
  std::string weight;
  std::string prefix;
  ...
  void configure(Config &c)
  {
    ...
    c("prefix", &prefix)("entity-FSA accepted tokens will be prefixed by this string (the open/close token tags are the same). empty=<eps>").self_init();
    c("weight", &weight)("entering the entity-FSA as opposed to using a dictionary token has this cost (Weight).").self_init();
  }
...
struct SpliceStateOptions
{
  ...
  static std::string hgName(int i) {
    return i==kTargetHg?"target":"source";
  }
  static std::string stateName(int j) {
    return j==kFinal?"final":"start";
  }
  static std::string noStateString() {
    return sdl::lexical_cast<std::string>(kNoState);
  }
  static std::string stateHelp()
  {
    return " (for default (start/final): "+noStateString()+")";
  }

  static std::string caption()
  {
    return "State Ids for splice"+stateHelp();
  }
...
{
  //nostate=empty
  void setPrefix(SplicePrefixOptions const& prefixOpt)
  {
    SplicePrefixOptions::operator = (prefixOpt);
  }
  void setStates(SpliceStateOptions const& stateOpt)
  {
    SpliceStateOptions::operator = (stateOpt);
  }
  void defaults()
  {
    SplicePrefixOptions::defaults();
...
/// embeds the part of the source hg between SpliceStateOptions.getStartFinal(kSourceHg) start->final into target hg getStartFinal(kTargetHg) both hg must have same vocab. note that start->final need not be the actual hg start and final state. actually copies ALL the source states.
template <class A>
struct Splice : TransformBase<Transform::Inplace, Transform::NoProperties>
{
  ...
  Splice(PHG const& hgSource, FullSpliceOptions const& opt) : hgSource(hgSource), spliceStates(opt), prefixOpt(opt)
  {
  }
  ...
  void inplace(IMutableHypergraph<A> &tar) const {
    ...
    if (srcStart==srcFinal) {
      if (tarStart==tarFinal && prefix.trivial()) {
        ...
        tar.addArcFsa(tarStart, tarFinal, EPSILON::ID, prefix.weight); // since prefix is empty, we can put it on the end
      } else {
