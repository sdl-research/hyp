...
struct WordListOptions : Util::LineOptions {
  ...
  WordListOptions()
      : chars(true), wordsep(" "), maxlines((std::size_t)-1), unigram_addk(.001), lengthBase(1), maxLength(10)
      , counts(true)
...
template <class W>
void readWordList(std::istream &in
                  , WeightedStrings<W> &ws
                  , WordListOptions const& opt = WordListOptions(), bool unweighted = false)
{
  assert(ws.size()==0);
      ...
      if (opt.chars) {
        if (!(in>>tok)) {
          ...
          SDL_THROW_LOG(TrieWordList, TrieWordListException,
                        "reading word list: got count " << c
                        <<" but no word (looking for non-whitespace utf8 chars on line " << i<<")");
        }
        opt.normalize(tok);
        ws.openString();
        ...
        if (!opt.getlineNormalized(in, tok)) {
          ...
          SDL_THROW_LOG(TrieWordList, TrieWordListException,
                        "reading word list: got count " << c
                        <<" but no word (looking for whitespace separated tokens on line " << i<<")");
        }
        ws.openString();
        graehl::split_noquote(tok, Util::predicateReference(ws), opt.wordsep);
      }
